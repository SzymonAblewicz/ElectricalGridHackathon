...
\section{Shift factor, sensitivity, PTDF}

\subsection*{The shift factor, as it is used today}

For a branch $e$ and a generator at node $n$, the shift factor is the change in
flow on $e$ per MW of injection change at $n$:
\begin{equation}
  \text{SF}_{e,n} \;=\; \frac{\Delta f_e}{\Delta P_n}.
  \label{eq:sf}
\end{equation}
[WDT] (Appendix 2, p.\,71) obtains this \emph{empirically}: move one generator
by a fixed amount, re-run the load flow, read the change in branch current.
One number, one branch, one generator, one solve.

\subsection*{The sensitivity --- what is computed here}

The same quantity, signed so that positive always means \emph{relief} of the
branch it belongs to:
\begin{equation}
  S_{e,n} \;=\; \text{PTDF}_{e,n}\cdot\operatorname{sign}(f_e).
  \label{eq:sens}
\end{equation}
The sign correction is what makes a row comparable across branches: without it
a positive entry means ``flow increases in the branch's reference direction'',
which is relief on one branch and overload on another. With it, positive means
relief everywhere, and the entries of a row may be ranked and summed.

\subsection*{The PTDF --- what carries the network}

Everything network-dependent sits in one matrix, the power transfer
distribution factor:
\begin{equation}
  \text{PTDF} \;=\; B K\T \Lp,
...
injected at bus $n$ and withdrawn across the slack. It is built from topology
and reactance alone --- $K$, $B$ and $L$ are defined next --- so it does not
depend on the hour, the weather or the dispatch. Only the $\operatorname{sign}(f_e)$
in \eqref{eq:sens} does.

\medskip
\textbf{Why this is available in closed form.} The AC power-flow equations are
nonlinear. The DC approximation linearises them under three assumptions ---
bus voltage magnitudes held at $1$ per unit; resistance negligible beside
reactance, $r \ll x$, so losses vanish; and small angle differences, so
$\sin(\theta_i-\theta_j)\approx\theta_i-\theta_j$ --- which makes the flow
exactly linear in the angle difference:
\begin{equation}
  f_e \;=\; \frac{\theta_{i(e)} - \theta_{j(e)}}{x_e}
       \;=\; b_e\,(\theta_{i} - \theta_{j}),
  \qquad b_e \equiv 1/x_e .
\end{equation}
Linearity is the whole reason a closed-form sensitivity exists. Where
\eqref{eq:sf} needs one perturb-and-re-solve per generator, \eqref{eq:ptdf}
returns every branch--node pair at once from a single inversion, exact to the
...
\medskip
\note{Every matrix that follows is from the real all-island network:
$\gnb$ buses, $\gne$ branches ($\gnlines$ lines and $\gntx$ transformers),
$\gnwindgen$ wind farms at $\gnwind$ buses. Nothing is a reduced example.}

\newpage
\section{Stage 1 --- build \texorpdfstring{$K$, $B$, $\varphi$, $L$}{K, B, phi, L}}

Under DC the grid is an ordinary resistive network and all three matrices are
...
conductance $G = 1/R$ & susceptance $b = 1/x$ \quad (this is $B$)\\
...
\begin{center}\includegraphics[width=\linewidth]{figures/m_K}\end{center}
...
same $+1/-1$ pair.
\begin{center}\input{generated/w_K}\end{center}
...
congested branches:
\[ B_{\text{cong}} = \input{generated/w_B} \]
...
\begin{center}\includegraphics[width=\linewidth]{figures/m_L}\end{center}

\newpage
\subsection*{$L$ --- real entries \quad ($\times 10^{3}$ MW/rad)}

\begin{center}\input{generated/w_L}\end{center}
...
\begin{center}\includegraphics[width=\linewidth]{figures/m_Lp}\end{center}
...
\begin{center}\input{generated/w_Lp}\end{center}
...
\newpage
\begin{center}\includegraphics[width=\linewidth]{figures/m_PTDF}\end{center}
...
\begin{center}\input{generated/w_PTDF_mixed}\end{center}
...
\begin{center}\includegraphics[width=\linewidth]{figures/m_PTDFwind}\end{center}
...
\begin{center}\input{generated/w_f}\end{center}
...
\begin{center}\includegraphics[width=\linewidth]{figures/m_sens}\end{center}

\subsection*{Sensitivity --- real entries}

\begin{center}\input{generated/w_sens}\end{center}
...
\begin{center}\includegraphics[width=\linewidth]{figures/f_heatmap}\end{center}
\note{Every congested branch against every wind node. Each row is a
constraint group as [WDT] would draw it --- read left to right, in isolation.
The blindness is visible: a node blue on one row is often red on another, and
the method that picked it never looked at that second row.}
...
\begin{center}\includegraphics[width=\linewidth]{figures/f_reach}\end{center}
\note{Left: how many congested branches each wind node reaches. Right: its net
weighted relief, \eqref{eq:netsens}, on the same map. A node dark on the left
and pale on the right is broad but pulling both ways at once --- which is
exactly the case counting rows cannot distinguish and summing them can.}
...
\medskip
\begin{center}\includegraphics[width=\linewidth]{figures/f_cascade}\end{center}
\note{One outage, side by side: line \texttt{1701-1981-1} trips, and
\texttt{3581-89516-1} goes to $107.6\%$ of its rating. \textbf{Left,} the
shift-factor group: it curtails $744$\,MW --- $14\%$ of the fleet --- over
three rounds and still loses two lines, \texttt{1122-1742-1} then
\texttt{1121-1301-1}, the first of them broken by the remedy itself.
\textbf{Right,} net sensitivity with the veto: $16$\,MW at a handful of nodes,
one round, nothing lost. A $46\times$ difference in curtailment on the same
fault. Row \texttt{1701-1981-1}, hour 50 of \texttt{cascade\_ranked.csv};
unarrested, the same fault takes 17 lines and islands the network.}
